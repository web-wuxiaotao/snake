...
function mouseWheel(obj,up,down){
	if(obj.attachEvent){                               
		document.attachEvent("onmousewheel",scrollFun)         //滚轮事件，IE
	}else if(obj.addEventListener){
		document.addEventListener("mousewheel",scrollFun,false)     //滚轮事件谷歌
		document.addEventListener("DOMMouseScroll",scrollFun,false) //滚轮事件火狐
	}
	function scrollFun(e){
		...
		var nub=e.wheelDelta||e.detail							//判断滚轮方向
		if(nub==120||nub==-3){									//滚轮方向为上
			up.call(obj)									//改变this指针，让this指针指向obj
		}else if(nub==-120||nub==3){							//滚轮方向为下
			down.call(obj)
		}
	}
}
